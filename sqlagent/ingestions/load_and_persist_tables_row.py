...
async def arun(
    sql_database: SQLDatabase, pinecone_api_key: str = os.getenv("PINECONE_API_KEY", ""), 
    ...
    for table_name in sql_database.get_usable_table_names():
        ...
        pinecone_vector_store = PineconeVectorStore(pinecone_index=pc_index, namespace=table_name)
        ...
        if table_name in indexed_tables:
            ...
            vector_index_dict[table_name] = VectorStoreIndex.from_vector_store(vector_store=pinecone_vector_store, embed_model=embedding_model)
            continue
        ...
        index = VectorStoreIndex(nodes=nodes, storage_context=storage_context, show_progress=True, embed_model=embedding_model)
